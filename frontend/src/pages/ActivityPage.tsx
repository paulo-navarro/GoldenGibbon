...
import { useOrders, useTrades, useTradeStats } from '../api';
import { useOrdersStore } from '../stores/ordersStore';
import { useTradesStore } from '../stores/tradesStore';
import type { Order } from '../types/orders';
import type { OrderSide, OrderStatus, ExitReason } from '../types/enums';
...
function OrderFilters({ filters, onChange }: { filters: OrderFiltersState; onChange: (f: OrderFiltersState) => void }) {
...
function applyOrderFilters(orders: Order[], filters: OrderFiltersState): Order[] {
  return orders
    .filter((o) => {
...
function OrdersSection() {
  const [filters, setFilters] = useState<OrderFiltersState>({ symbol: '', side: '', status: '' });
  ...
  const filtered = useMemo(() => applyOrderFilters(allOrders, filters), [allOrders, filters]);
  ...
  if (error) {
    return <Alert severity="error" variant="outlined">Failed to load orders</Alert>;
  }
